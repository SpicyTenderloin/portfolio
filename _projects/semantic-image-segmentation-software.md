...
category: university
thumbnail: /assets/img/semantic-image-segmentation-software/classical-pipeline.jpg
skills:
  - Image Processing
  - Computer Vision
  - Machine Learning
...
**Approach:** my own contribution was the classical segmentation pipeline and the performance-metric calculations used to evaluate every method in the report. The classical pipeline converts each image into the perceptually uniform CIELAB colour space, smooths it with a mean-shift filter to suppress false edges while preserving real boundaries, enhances local contrast with adaptive histogram equalisation to bring out weak edges in dark regions, then runs Canny edge detection with thresholds set dynamically from each image's own gradient statistics rather than a single fixed threshold, since the right threshold varies a lot between scenes. A final pass of morphological closing, dilation, and region-filling turns the resulting edge map into clean, enclosed regions. A teammate developed a ResNet-U-Net machine learning model for the same task, later extended with a depth channel through mid-level fusion.

<figure>
  <a class="lightbox-trigger" href="{{ "/assets/img/semantic-image-segmentation-software/classical-pipeline.jpg" | relative_url }}">
    <img src="{{ "/assets/img/semantic-image-segmentation-software/classical-pipeline.jpg" | relative_url }}" alt="The classical segmentation pipeline shown stage by stage: input, mean-shift smoothing, contrast enhancement, Canny edge detection, morphological refinement, and final filled regions">
  </a>
  <figcaption>The classical pipeline shown stage by stage on a real test image</figcaption>
</figure>
